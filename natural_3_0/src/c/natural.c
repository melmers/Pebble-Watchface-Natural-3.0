...
static TextLayer *time_text_layer, *date_text_layer, *temp_text_layer, *day_text_layer, *month_text_layer;

static char time_buffer[16], date_buffer[16], temp_buffer[16], day_buffer[16], month_buffer[16], log_buffer[256];
...
static void minute_tick_handler(struct tm *tick_time, TimeUnits units_changed) {
  ...
  strftime(month_buffer, sizeof("000"), "%h", tick_time);
...
static void window_load(Window *window) {
  ...
  date_text_layer = init_text_layer(GRect(0, 136, 50, 32), GColorWhite, GColorClear, FONT_KEY_DROID_SERIF_28_BOLD, GTextAlignmentLeft);
  text_layer_set_text(date_text_layer, "00-00");
  layer_add_child(window_layer, (Layer*) date_text_layer);

  // Create the month text layer
  // month_text_layer = init_text_layer(GRect(0, 141, 50, 24), GColorWhite, GColorClear, FONT_KEY_GOTHIC_24_BOLD, GTextAlignmentLeft);
  // text_layer_set_text(month_text_layer, "000");
  // layer_add_child(window_layer, (Layer*) month_text_layer);
